  ...
  return (
      <div className='flex flex-col justify-center bg-gradient-to-tr from-background to-background/50 w-full items-center h-screen p-4 gap-4'>
          ...
          <div className='relative h-full flex flex-col justify-center w-full max-w-md'>
            ...
            <p className='text-copy-light font-light text-sm mb-5'>Don't have an account yet? <a href="/register" className='underline underline-offset-2 text-copy hover:text-copy/80 duration-200 transition'>Create a account for free</a></p>

            <input value={email} onChange={(e) => setEmail(e.target.value)}
                className='autofill-style mt-3 bg-foreground rounded-md px-4 py-3 outline-none border border-border focus:border-primary duration-200 transition-all text-sm' placeholder='Email Address'
                type="email"/>
            <input value={password} onChange={(e) => setPassword(e.target.value)}
                className='autofill-style mt-3 bg-foreground rounded-md px-4 py-3 outline-none border border-border focus:border-primary duration-200 transition-all text-sm' placeholder='Password'
                type="password"/>
